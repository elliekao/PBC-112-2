    ...
    for news_item in news_list:
        ...
        for keyword in keywords: # 檢查所有關鍵字
            if keyword in title: # 如果標題中包含關鍵字
                filtered_news.append({'title': title, 'url': url})  # 添加新聞標題與URL到過濾的清單
                break
...
def get_article_word_count(url):
    try:
        ...
        section = soup.find('section', class_='article-content__editor') # 找到文章內容的主要區域
        ...
        if section:
            ...
            word_count = len(re.findall(chinese_char_pattern, article))  # 計算中文字元數量
...
if __name__ == "__main__":
    ...
    top_news = find_top_articles(filtered_news, keywords, top_n=3) # 再從裡面找到前3篇包含最多關鍵字的新聞，若關鍵字數重複則以文章內容字數來做第三階段篩選
